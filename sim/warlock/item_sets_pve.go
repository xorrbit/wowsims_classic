...
var ItemSetDemoniacsThreads = core.NewItemSet(core.ItemSet{
	...
	Bonuses: map[int32]core.ApplyEffect{
		...
		3: func(agent core.Agent) {
			warlock := agent.(WarlockAgent).GetWarlock()
			for _, corr := range warlock.Corruption {
				corr.DamageMultiplierAdditive += 0.02
			}
		},
...
var ItemSetDoomcallersAttire = core.NewItemSet(core.ItemSet{
	...
	Bonuses: map[int32]core.ApplyEffect{
		...
		4: func(agent core.Agent) {
			warlock := agent.(WarlockAgent).GetWarlock()
			for _, corr := range warlock.Immolate {
				corr.BaseDamageMultiplierAdditive += 0.05
			}
		},
...
var ItemSetPlagueheartRaiment = core.NewItemSet(core.ItemSet{
	...
	Bonuses: map[int32]core.ApplyEffect{
		...
		4: func(agent core.Agent) {
			warlock := agent.(WarlockAgent).GetWarlock()
			for _, corr := range warlock.Corruption {
				corr.DamageMultiplierAdditive += 0.12
			}
		},
